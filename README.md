...
### Shared player customization

The shared player follows the system light/dark appearance, including its page background, card, text, controls, and waveform. Space toggles playback even when the waveform, volume, play button, or version picker is focused. The waveform has no focus outline; arrow-key seeking still works. The local waveform also accepts Space without intercepting its arrow keys.

Open **Settings → Customize shared player**, or visit `http://localhost:53038/ui-editor`, for the local-only design studio. It previews the same HTML and theme generator used by the Worker, with separate light/dark colors, layout and typography sliders, font choices, intro text, visibility toggles, and desktop/tablet/phone widths. Drafts save in browser storage. Undo/redo, reset, JSON import/export, and CSS copying are available. Optional local audio stays in the browser; the initial preview uses a generated demo tone.

Export the finished `player-theme.json` and replace `worker/player-theme.json` with it to apply the design. Run `pnpm check`, `pnpm test`, and `pnpm share:deploy` (Node 22), then commit and push. Editing a draft does not change published links. `worker/player-theme.ts` defines the validated customization options; `server/ui-editor/` contains the temporary editor and is not shipped to Cloudflare. Keep the preview on `playerPage()` so it stays faithful to the deployed player.
